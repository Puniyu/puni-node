...
export class YamlEditor {
  ...
  get (path: string) {
    try {
      ...
      return get(this.document.toJSON(), path)
    } catch (error) {
  ...
  remove (path: string, value: YamlValue, isSplit = true): boolean {
    try {
      ...
      const index = current.items.findIndex((item) => isEqual(item.toJSON(), value))
      if (index < 0) {
      ...
      logger.debug(
        `[YamlEditor] 已从 ${path} 数组删除元素：${typeof value === 'object' ? JSON.stringify(value) : value
        }`
      )
      return true
    } catch (error) {
  ...
  hasval (path: string, value: YamlValue, isSplit = true): boolean {
    try {
      ...
      if (current instanceof YAML.YAMLSeq) {
        /** 如果是序列，遍历序列查找值 */
        return current.items.some((item) => isEqual(item.toJSON(), value))
      } else if (current instanceof YAML.YAMLMap) {
        /** 如果是映射，检查每个值 */
        return Array.from((current as any).values()).some((v: any) => isEqual(v.toJSON(), value))
      } else {
        /** 否则，直接比较值 */
