 ...
 */
const fnStore = new Map<string, { key: string; fnc: Function }>()
 ...
 * @returns 唯一标识符 用于解锁
 */
export const lockMethod = <T extends object, K extends keyof T>(
  obj: T,
  key: K,
  msg: string = `方法 ${String(key)} 已被锁定`
) => {
  ...
  /** 唯一标识符 用于解锁 */
  const storeKey = randomUUID().replace(/-/g, '')
  /** 保存原有函数 */
  fnStore.set(storeKey, { key: String(key), fnc: obj[key] as Function })
 ...
 * @returns 解锁结果
 */
const unlockMethod = <T extends object>(obj: T, storeKey: string) => {
