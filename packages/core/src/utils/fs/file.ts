...
import { puniPathBase } from '@puniyu/root'
import type { DownloadFileResult, DownloadFilesOptions } from '@puniyu/types'
import { existToMkdir } from '@puniyu/utils/fs/fsPromise'
...
): Promise<DownloadFileResult<boolean>> => {
 ...
 * @param dirpath 文件夹路径
 */
export const createBaseDirs = async (dirs: string[], dirpath: string): Promise<void> => {
  if (!dirs) throw new Error('文件夹列表不能为空')
  if (!dirpath) throw new Error('文件夹路径不能为空')
  await Promise.all(dirs.map(async (dir) => {
    const filePath = path.join(dirpath, dir)
    await existToMkdir(filePath)
  }))
}
...
export const createPluginDir = async (name: string, files?: string[]): Promise<void> => {
  ...
  const pluginPath = isOrgPkg
    ? path.join(puniPathBase, ...name.split('/'))
    : path.join(puniPathBase, name)

  return createBaseDirs(files, pluginPath)
}
 ...
 * @param files 需要创建的文件夹列表
 */
export const createAdapterDir = async (name: string, files?: string[]): Promise<void> => {
  if (!Array.isArray(files)) files = ['config', 'data', 'resources']
  if (files.length === 0) return

  const adapterName = name.replace(/^(?:@[^/]+[/])?adapter-/, '$1')
  const adapterPath = path.join(puniPathBase, adapterName)
  return createBaseDirs(files, adapterPath)
}
 ...
 * @param files 需要创建的文件夹列表
 */
export const createRenderDir = async (name: string, files?: string[]): Promise<void> => {
  if (!Array.isArray(files)) files = ['config', 'data']
  if (files.length === 0) return

  const adapterName = name.replace(/^(?:@[^/]+[/])?render-/, '$1')
  const adapterPath = path.join(puniPathBase, adapterName)
  return createBaseDirs(files, adapterPath)
}
