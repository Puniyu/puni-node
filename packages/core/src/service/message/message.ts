import type { CustomNodeElement, Elements, SendMessage } from '@puniyu/types/service/message/segment'
  ...
  const rawStr = data
    .map((v) => {
      switch (v.type) {
        ...
        default:
          return `[未知:${JSON.stringify(v).slice(0, 200)}${JSON.stringify(v).length > 200 ? '...' : ''}]`
      }
    })
    .join('')
  return {
